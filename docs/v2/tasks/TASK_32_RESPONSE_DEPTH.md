...
- [ ] Generator получает `response_depth` в payload.
- [ ] Обычный вопрос классифицируется как `concise`.
- [ ] `Поясни детальнее, не понятно` классифицируется как `clarify`.
- [ ] `Расскажи подробнее` классифицируется как `detail`.
- [ ] Явный глубокий запрос классифицируется как `deep`.
- [ ] Personal prompt знает контракт всех четырёх режимов.
- [ ] Group prompt знает контракт всех четырёх режимов.
- [ ] Удалено старое допущение, что `объясни нормально` само по себе разрешает многоэкранный разбор.
- [ ] Unit regression tests зелёные.
- [ ] Full `tests_v2` зелёный до merge.
