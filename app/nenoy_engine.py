...
RESPONSES: dict[str, tuple[str, ...]] = {
    ...
    "overplanning": (
        ...
        "Сначала не карта, а точка контакта с действием. {goal_line} Выбери один микро-действиe и запусти его. {closing}",
    ),
    "motivation_request": (
    ...
    "deadline_missing": (
        "Шаг есть. Срока нет. {goal_line} Без времени это намерение, а не обязательство. {closing}",
    ...
    "procrastination": (
        "Не надо делать всё. {goal_line} Сделай первый кусок за 10 минут. {closing}",
        "Сейчас задача страшнее слов про неё. {goal_line} 2 минуты: один вход, один факт, потом перехватиём движение. {closing}",
        "Прокрастинация громче, когда ты ждёшь идеала. {goal_line} Выбирай конкретный минимальный шаг и запусти его. {closing}",
    ),
    "fatigue": (
        "Устал — это факт, не приговор. {goal_line} Но нить не рвём: открой задачу на 20 секунд, найди точку входа и закрывай. ⛓️",
        "Энергия просела. {goal_line} Снимаем героизм: 20 секунд контакта с задачей, один вход, один факт. ⛓️",
        "Да, ресурс ниже. {goal_line} Сначала символический вход: открой задачу и зафиксируй первый файл. ⛓️",
    ),
    "postpone": (
    ...
    "off_topic": (
        "Интересно, но это не маршрут. {goal_line} Вернись к задаче и назови следующий шаг. {closing}",
        "Офтоп не переносится в действие. {goal_line} Назови фактически сделанный шаг или возвращайся к цели. {closing}",
    ),
    "goal_focus": (
...
def _pick_closing_phrase(recent_messages: tuple[tuple[str, str], ...], user_message: str) -> str:
    ...
    return CLOSING_PHRASES_DEFAULT[0]
